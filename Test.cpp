...
double rstart = 0.09; //ECCENTRICITY VAR
double e = 6;
...
//double v_gas[] = { 0.2, 0.5, 0.8 };
double v_gas[] = { 0.3, 0.5, 0.7 };  //-0.7, 0.9, 1E7, 1E2, 1/4
//double v_gas[] = { 0.9 };
//double discgas_fraction[] = { 0.3, 0.5, 0.9 }; /* rc in  kpc */
double discgas_fraction[] = { 0.3, 0.5, 0.9 };
//double discgas_fraction[] = { 0.1 };
...
double total_mass[] = { 3.0E+8, 3.0E+7, 5.0E+7 }; 
//double total_mass[] = { 1.0E+8 };


double rho_gas[] = { 3.0E+2, 2.0E+2, 1.0E+2 }; /* central gas particle /cm^3 at rc */

//double rho_gas[] = { 1.0E+2 };
...
double mass_ratio[] = { 1.0 / 2.0, 1.0 / 3.0, 1.0 / 4.0 };
//double mass_ratio[] = { 1.0/2.0 };
...
double* ddr(double M1, double M2, double rho, double fg, double vg, double t, double r_vector_0, double r_vector_1, double dr_vector_0, double dr_vector_1) {
	...
	/*double vc = pow((6.67E-11*M_total*2E+30/(r*3.086E+19)),0.5);*/ //EDITEDBELOW!!!!!!!!!!!!!!!!!!!!!!
...
double* solve_eom(double M1, double M2, double gas, double fg, double vg, double t0, double r0_r, double r0_phi, double dr0_r, double dr0_phi, double rf_r, double rf_phi, int knd, int jnd, int nnd, int ind, int vnd, double st, double af, double ai, double lf)
{
	...
	sprintf(filename, "data_%ld.txt", time(NULL));
	...
	while ((r0[0] >= af) && (st < tcos)) {
		...
		st = st + h;
		t0 = t0 + h;/* in second*/
		//printf ("%E\t", t0/(3600.0*24*365)); // YOU CHANGED THIS!!!!!!!!!!!!
		double x, y, vx, vy;
...
double* Decay(double z, double q, double gas, double fg, double Mtot, double vg, int knd, int jnd, int nnd, int ind, int vnd)
{
	...
	double vi_e = -1 * vi * pow(((1.0 + e)), 0.5); /* start from the peri-center */ //CHANGE DIRECTION
...
int main() {
	...
	double** phase_time_1 = new double*[n_q];
	for (int i = 0; i < n_q; i++)
		phase_time_1[i] = new double[1];
	...
	double** n1_ratio_b = new double* [n_gas];
	for (int i = 0; i < n_gas; i++)
		n1_ratio_b[i] = new double[n_q];
	//double n2_ratio_b[n_gas][n_q]; /* the fraction of a_ratio*[dt2/(dt1+dt2+dt3+dt4)] for different q */
	double** n2_ratio_b = new double* [n_gas];
	for (int i = 0; i < n_gas; i++)
		n2_ratio_b[i] = new double[n_q];
	//double n3_ratio_b[n_gas][n_q]; /* the fraction of a_ratio*[dt3/(dt1+dt2+dt3+dt4)] for different q */
	double** n3_ratio_b = new double* [n_gas];
	for (int i = 0; i < n_gas; i++)
		n3_ratio_b[i] = new double[n_q];
	//double n4_ratio_b[n_gas][n_q]; /* the fraction of a_ratio*[dt4/(dt1+dt2+dt3+dt4)] for different q */
	double** n4_ratio_b = new double* [n_gas];
	for (int i = 0; i < n_gas; i++)
		n4_ratio_b[i] = new double[n_q];
	double c_ratio[3] = { 1.0, 1.0, 1.0 }; /* the fraction of different total mass */
	//double n1_ratio_c[n_Mtot][n_gas][n_q];
	double*** n1_ratio_c = new double** [n_Mtot];
	for (int i = 0; i < n_Mtot; ++i) {
		n1_ratio_c[i] = new double* [n_gas];
		for (int j = 0; j < n_gas; ++j) {
			n1_ratio_c[i][j] = new double[n_q];
		}
	}
	//double n2_ratio_c[n_Mtot][n_gas][n_q];
	double*** n2_ratio_c = new double** [n_Mtot];
	for (int i = 0; i < n_Mtot; ++i) {
		n2_ratio_c[i] = new double* [n_gas];
		for (int j = 0; j < n_gas; ++j) {
			n2_ratio_c[i][j] = new double[n_q];
		}
	}
	//double n3_ratio_c[n_Mtot][n_gas][n_q];
	double*** n3_ratio_c = new double** [n_Mtot];
	for (int i = 0; i < n_Mtot; ++i) {
		n3_ratio_c[i] = new double* [n_gas];
		for (int j = 0; j < n_gas; ++j) {
			n3_ratio_c[i][j] = new double[n_q];
		}
	}
	//double n4_ratio_c[n_Mtot][n_gas][n_q];
	double*** n4_ratio_c = new double** [n_Mtot];
	for (int i = 0; i < n_Mtot; ++i) {
		n4_ratio_c[i] = new double* [n_gas];
		for (int j = 0; j < n_gas; ++j) {
			n4_ratio_c[i][j] = new double[n_q];
		}
	}
	//double n_ratio_c[n_Mtot][n_gas][n_q];
	double*** n_ratio_c = new double** [n_Mtot];
	for (int i = 0; i < n_Mtot; ++i) {
		n_ratio_c[i] = new double* [n_gas];
		for (int j = 0; j < n_gas; ++j) {
			n_ratio_c[i][j] = new double[n_q];
		}
	}

	double d_ratio[3] = { 1.0, 1.0, 1.0 }; /* the fraction of different total mass */
	//double n1_ratio_d[n_fg][n_Mtot][n_gas][n_q];
	double**** n1_ratio_d = new double*** [n_fg];
	for (int i = 0; i < n_fg; ++i) {
		n1_ratio_d[i] = new double** [n_Mtot];
		for (int j = 0; j < n_Mtot; ++j) {
			n1_ratio_d[i][j] = new double* [n_gas];
			for (int k = 0; k < n_gas; ++k) {
				n1_ratio_d[i][j][k] = new double[n_q];
			}
		}
	}
	//double n2_ratio_d[n_fg][n_Mtot][n_gas][n_q];
	double**** n2_ratio_d = new double*** [n_fg];
	for (int i = 0; i < n_fg; ++i) {
		n2_ratio_d[i] = new double** [n_Mtot];
		for (int j = 0; j < n_Mtot; ++j) {
			n2_ratio_d[i][j] = new double* [n_gas];
			for (int k = 0; k < n_gas; ++k) {
				n2_ratio_d[i][j][k] = new double[n_q];
			}
		}
	}
	//double n3_ratio_d[n_fg][n_Mtot][n_gas][n_q];
	double**** n3_ratio_d = new double*** [n_fg];
	for (int i = 0; i < n_fg; ++i) {
		n3_ratio_d[i] = new double** [n_Mtot];
		for (int j = 0; j < n_Mtot; ++j) {
			n3_ratio_d[i][j] = new double* [n_gas];
			for (int k = 0; k < n_gas; ++k) {
				n3_ratio_d[i][j][k] = new double[n_q];
			}
		}
	}
	//double n4_ratio_d[n_fg][n_Mtot][n_gas][n_q];
	double**** n4_ratio_d = new double*** [n_fg];
	for (int i = 0; i < n_fg; ++i) {
		n4_ratio_d[i] = new double** [n_Mtot];
		for (int j = 0; j < n_Mtot; ++j) {
			n4_ratio_d[i][j] = new double* [n_gas];
			for (int k = 0; k < n_gas; ++k) {
				n4_ratio_d[i][j][k] = new double[n_q];
			}
		}
	}
	//double n_ratio_d[n_fg][n_Mtot][n_gas][n_q];
	double**** n_ratio_d = new double*** [n_fg];
	for (int i = 0; i < n_fg; ++i) {
		n_ratio_d[i] = new double** [n_Mtot];
		for (int j = 0; j < n_Mtot; ++j) {
			n_ratio_d[i][j] = new double* [n_gas];
			for (int k = 0; k < n_gas; ++k) {
				n_ratio_d[i][j][k] = new double[n_q];
			}
		}
	}
	...
	double***** n1_ratio_e = new double**** [n_vg];
	for (int i = 0; i < n_vg; ++i) {
		n1_ratio_e[i] = new double*** [n_fg];
		for (int j = 0; j < n_fg; ++j) {
			n1_ratio_e[i][j] = new double** [n_Mtot];
			for (int k = 0; k < n_Mtot; ++k) {
				n1_ratio_e[i][j][k] = new double* [n_gas];
				for (int l = 0; l < n_gas; ++l) {
					n1_ratio_e[i][j][k][l] = new double[n_q];
				}
			}
		}
	}
	//double n2_ratio_e[n_vg][n_fg][n_Mtot][n_gas][n_q];
	double***** n2_ratio_e = new double**** [n_vg];
	for (int i = 0; i < n_vg; ++i) {
		n2_ratio_e[i] = new double*** [n_fg];
		for (int j = 0; j < n_fg; ++j) {
			n2_ratio_e[i][j] = new double** [n_Mtot];
			for (int k = 0; k < n_Mtot; ++k) {
				n2_ratio_e[i][j][k] = new double* [n_gas];
				for (int l = 0; l < n_gas; ++l) {
					n2_ratio_e[i][j][k][l] = new double[n_q];
				}
			}
		}
	}
	//double n3_ratio_e[n_vg][n_fg][n_Mtot][n_gas][n_q];
	double***** n3_ratio_e = new double**** [n_vg];
	for (int i = 0; i < n_vg; ++i) {
		n3_ratio_e[i] = new double*** [n_fg];
		for (int j = 0; j < n_fg; ++j) {
			n3_ratio_e[i][j] = new double** [n_Mtot];
			for (int k = 0; k < n_Mtot; ++k) {
				n3_ratio_e[i][j][k] = new double* [n_gas];
				for (int l = 0; l < n_gas; ++l) {
					n3_ratio_e[i][j][k][l] = new double[n_q];
				}
			}
		}
	}
	//double n4_ratio_e[n_vg][n_fg][n_Mtot][n_gas][n_q];
	double***** n4_ratio_e = new double**** [n_vg];
	for (int i = 0; i < n_vg; ++i) {
		n4_ratio_e[i] = new double*** [n_fg];
		for (int j = 0; j < n_fg; ++j) {
			n4_ratio_e[i][j] = new double** [n_Mtot];
			for (int k = 0; k < n_Mtot; ++k) {
				n4_ratio_e[i][j][k] = new double* [n_gas];
				for (int l = 0; l < n_gas; ++l) {
					n4_ratio_e[i][j][k][l] = new double[n_q];
				}
			}
		}
	}
	//double n_ratio_e[n_vg][n_fg][n_Mtot][n_gas][n_q];
	double***** n_ratio_e = new double**** [n_vg];
	for (int i = 0; i < n_vg; ++i) {
		n_ratio_e[i] = new double*** [n_fg];
		for (int j = 0; j < n_fg; ++j) {
			n_ratio_e[i][j] = new double** [n_Mtot];
			for (int k = 0; k < n_Mtot; ++k) {
				n_ratio_e[i][j][k] = new double* [n_gas];
				for (int l = 0; l < n_gas; ++l) {
					n_ratio_e[i][j][k][l] = new double[n_q];
				}
			}
		}
	}
			...
			for (k = 1; k <= n_Mtot; ++k) {
				...
				for (j = 1; j <= n_gas; ++j) {
					...
					for (i = 1; i <= n_q; ++i) {
						...
						L1 = L1 + (dt1 * pow(10.0, (p1 * log10(a1) + p2))) * pow(10.0, (p3 * log10(mass_ratio[ind]) + p4));
					} //!!!!!!!!!!!!!YOU CHANGED f_dt1 to dt1
